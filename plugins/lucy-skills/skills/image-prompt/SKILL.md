...
> **Adaptasi Lucy (2026-07-18)** — dari `Feature/Image-Prompt-System` (upstream Kiyoraka), user-scope.
> Skill ini jana **TEKS prompt sahaja** (Midjourney/NijiJourney) — **TIADA panggilan API, tiada kos, selamat**.
> Untuk jana imej SEBENAR perlu `Image-Generation-System` (API berbayar) yang **sengaja tidak dipasang**.
> **Character Mode** baca `## Character Appearance` dari fail identiti Lucy (`main/identity-core.md`)
> jika seksyen itu wujud; kalau tiada → terus guna **Freeform Mode** (jangan reka ciri).
