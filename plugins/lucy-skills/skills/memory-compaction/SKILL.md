...
> **Adaptasi Lucy (2026-07-17)** — dari upstream Kiyoraka PR #9 (aimanmalib).
> Perubahan dari upstream ditanda `[LUCY]`. Selain itu, protokol kekal sama.
...
### [LUCY] Mengukur saiz pada Windows

```powershell
# Unit chars (cth main/current-session.md)
(Get-Content main\current-session.md -Raw).Length

# Unit lines (cth main/main-memory.md)
(Get-Content main\main-memory.md | Measure-Object -Line).Lines
```

Guna **unit yang policy nyatakan untuk fail itu** — jangan campur. Bajet `chars` diukur dengan
`.Length`, bajet `lines` diukur dengan `Measure-Object -Line`.
...
> **[LUCY] Untuk fail berbajet `chars`** (cth `main/current-session.md`): sempadan 30% dikira
> pada **jumlah aksara**, bukan baris. Guna unit yang sama dengan bajet fail itu — kalau tidak
> sempadan tier tak sepadan dengan bajet yang mencetuskannya.

> **[LUCY] Sempadan entri** — `main/current-session.md` tiada entri berbaris tunggal. Satu entri
> = satu **session record** (`### 🆕 Sesi YYYY-MM-DD ...` hingga `--- (rekod sesi lepas) ---`).
> Blok header fail (`# 🌟 Current Session Memory` + `## Session RAM Status`) **bukan** entri —
> ia kekal di puncak, tidak pernah dicompact. Lihat policy untuk definisi penuh.
...
> **[LUCY] Apa yang WAJIB hidup** dalam konteks kita — ini bukan "chatter", jangan buang:
> pengajaran teknikal (cth *"verify cetak guna `page.pdf()`, bukan element screenshot"*),
> gotcha deploy (cth *"custom domain = authoritative, workers.dev boleh lag"*), punca bug
> (cth *"PK komposit JOIN separuh kunci = baris berganda"*), dan keputusan master + sebabnya.
> Yang boleh dibuang: narasi langkah-demi-langkah, senarai commit hash penuh, status "siap/OK"
> berulang, dan konteks satu-kali yang dah selesai.
...
> **[LUCY] Peraturan #7 — JANGAN jalankan 500-Line Limit Protocol.**
> `main/session-format.md` mengandungi protokol lama yang **PADAM** detail dan kekalkan recap
> 20-30 baris sahaja. Skill ini **menggantikannya**. Kalau kedua-dua nampak terpakai, yang ini
> menang — sentiasa. Ringkaskan, jangan padam.
